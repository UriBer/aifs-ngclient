...
export class ConfigManager {
  ...
  async loadConfig(): Promise<AppConfig> {
    try {
      ...
      const config: AppConfig = JSON.parse(decryptedData);
  ...
  async getProviderConfig(scheme: string): Promise<ProviderConfig | null> {
  ...
  async enableProvider(scheme: string): Promise<void> {
    await this.updateProviderConfig(scheme, { enabled: true });
  }

  async disableProvider(scheme: string): Promise<void> {
    await this.updateProviderConfig(scheme, { enabled: false });
  }

  async setProviderCredentials(scheme: string, credentials: Record<string, string>): Promise<void> {
    await this.updateProviderConfig(scheme, { credentials });
  }

  async setProviderSettings(scheme: string, settings: Record<string, any>): Promise<void> {
  ...
  async validateProviderConfig(scheme: string): Promise<{ valid: boolean; errors: string[] }> {
    ...
    switch (scheme) {
      case 's3':
        if (!provider.credentials.accessKeyId) errors.push('Access Key ID is required');
        if (!provider.credentials.secretAccessKey) errors.push('Secret Access Key is required');
        if (!provider.credentials.region) errors.push('Region is required');
        if (!provider.credentials.bucket) errors.push('Bucket name is required');
        break;
      
      case 'gcs':
        if (!provider.credentials.projectId) errors.push('Project ID is required');
        if (!provider.credentials.bucket) errors.push('Bucket name is required');
        break;
      ...
      case 'az':
        ...
        if (!provider.credentials.containerName) errors.push('Container name is required');
        break;
      
      case 'aifs':
        if (!provider.credentials.endpoint) errors.push('Endpoint is required');
        break;
    }
